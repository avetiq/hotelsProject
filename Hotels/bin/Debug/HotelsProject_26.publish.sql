...
AS
	select *
	from (select h1.[Name] as HotelName,h1.City as CityName,h2.[Money] as Moneyy,(RoomQauntity-reservedCount) as tarberutyun
	from Hotels as h1
	join HotelsRoomsInfo as h2
	on h1.ID = h2.HotelId
	left join (select h2.ID,count(*) as reservedCount
			...
			where h1.Name = isnull(@HotelName,h1.Name) and h1.City = isnull(@CityName,h1.City) and
								h2.BedQuantity = isnull(@BedQuantity,h2.BedQuantity) and (
											(isnull(@From, b.[From]) <= b.[From] and isnull(@To, b.[To]) >= b.[From])
											OR (isnull(@From, b.[From]) >= b.[From] and isnull(@From, b.[From]) < b.[To])
													)
			group by h2.ID) as k
	on k.ID = h2.ID
	where  h1.Name = isnull(@HotelName,h1.Name) and h1.City = isnull(@CityName,h1.City) and
...
INSERT INTO booked
VALUES ('2020-03-06', '2020-03-15', 5, '1111'),
		('2020-03-02', '2020-03-14', 5, '2222'),
