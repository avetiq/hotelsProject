...
GO
PRINT N'Creating [dbo].[ExistsSuchHotel]...';


GO
CREATE FUNCTION [dbo].[ExistsSuchHotel]
(
...
RETURNS INT
AS
-- If the return value is 1, then there is already exsists such room
BEGIN
	DECLARE @ret int
...
END
GO
PRINT N'Creating [dbo].[ExistsSuchHotelBedQuantity]...';


GO
CREATE FUNCTION [dbo].[ExistsSuchHotelBedQuantity]
(
	@HotelID int,
	@BedQuantity int
)
...
BEGIN
	...
	where HotelId = @HotelID AND BedQuantity = @BedQuantity
...
--ete masnachyux chka,@HotID -n klini 0, piti insert anenq
	...
	Set @HotID = isnull((SELECT h.ID FROM Hotels h WHERE h.City = 'New York' AND h.[Name] = 'Hilllton Hotel'),0)
...
--hima piti stugenq tenc qanakov senyakner ka te voch
	...
	SELECT @EXISTS = dbo.ExistsSuchHotelBedQuantity(@HotID, @BedQuantity)
